...
export default function Topic({ item }) {
  return (
    ...
    >
      ...
      <div className='font-bold text-[15px] leading-5 mt-0.5'>
        {item.topic.type === 'tag' && '#'}
        {item.topic.value}
      </div>
      {item.postCount && (
      ...
      <Popover className='absolute top-1.5 right-2'>
        <Popover.Button className='outline-none w-[34.75px] h-[34.75px] rounded-full text-[#71767b] flex items-center justify-center hover:bg-[#1d9bf01a] hover:text-[#1d9bf0] transition-colors'>
          <svg viewBox='0 0 24 24' width={18.75} height={18.75} className=''>
            <path
              fill='currentColor'
        ...
        <Popover.Panel className='w-[384px] grid absolute top-2 right-0 overflow-hidden bg-black shadow-box rounded-xl z-10'>
          <button className='py-3 px-4 flex items-center text-left gap-3 text-[15px] transition-colors hover:bg-white/[0.05] leading-5 font-bold text-[#e7e9ea]'>
